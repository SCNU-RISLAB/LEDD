...
'''----------------------------------------------------------------------------'''
class MDKC_Module(nn.Module):
    #
    def __init__(self, c1, c2, n=1, kernels=[3,7], add_identity=True, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(*(MDKC_Block(c_, c_, kernels, add_identity) for _ in range(n)))

    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))
    
class MDKC_Layer(nn.Module):
    def __init__(self, in_channels, out_channels, kernels, add_identity=True):
        super().__init__()
        self.conv = nn.Sequential(nn.Conv2d(in_channels,out_channels,3,padding='same'),
                                  nn.GELU()
                                  )
        self.EMSConvPP = MDKC(channel=in_channels, out=out_channels, kernels=kernels)
...
class MDKC(nn.Module):
    def __init__(self, channel=256, out=256, kernels=[3,7]):
        super().__init__()
        self.groups = len(kernels)
        min_ch = channel // self.groups
        self.convs = nn.ModuleList([])
        for ks in kernels:
            self.convs.append(Conv(c1=min_ch, c2=min_ch, k=ks, g = min_ch))
        self.activation = nn.GELU()
        self.BatchNorm2d = nn.BatchNorm2d(out)
        
    def forward(self, x):
        batch_size, channels, height, width = x.shape
        x_group = x.view(batch_size, -1, height, width, self.groups)
        x_convs = torch.stack([self.convs[i](x_group[..., i]) for i in range(len(self.convs))])
        x_convs = x.view(batch_size, channels, height, width)
        x_convs = self.activation(x_convs)
        x_convs = self.BatchNorm2d(x_convs)
        return x_convs
    

class MLCF_Module(nn.Module):
    def __init__(self, d_model, out_channel):
            super().__init__()
            self.proj_1 = nn.Conv2d(d_model, d_model, 1)
            self.activation = nn.GELU()
            self.spatial_gating_unit = MLCF(d_model, d_model)
...
class MLCF(nn.Module):
    def __init__(self, in_channel,out_channel):
        super().__init__()
        c_ = out_channel // 2
        self.conv1_1 = Conv(in_channel,c_,1)
        self.cv1 = nn.Sequential(
                nn.Conv2d(c_, c_, kernel_size= 7, stride=(1,1), padding='same', groups=c_),
                nn.BatchNorm2d(c_),
                nn.Conv2d(c_, c_, kernel_size= 7, stride=(1,1), padding='same', groups=c_, dilation=2),
                nn.GELU()
        )

        self.cv2 = nn.Sequential(
                nn.Conv2d(c_, c_, kernel_size=11, stride=(1,1), padding='same', groups=c_),
                nn.BatchNorm2d(c_),
                nn.Conv2d(c_, c_, kernel_size=11, stride=(1,1), padding='same', groups=c_, dilation=2),
                nn.GELU()
        )

        self.cv3 = nn.Sequential(
                nn.Conv2d(c_, c_, kernel_size=21, stride=(1,1), padding='same', groups=c_),
                nn.BatchNorm2d(c_),
                nn.Conv2d(c_, c_, kernel_size=21, stride=(1,1), padding='same', groups=c_, dilation=2),
                nn.GELU()
        )
        self.m = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.conv1_1_2 = Conv(c_*4,out_channel,1)
    ...
    def forward(self, x):
        ...
        out2 = self.cv2(out)
        out3 = self.cv3(out)
        return self.m(self.conv1_1_2(torch.cat((out,out1,out2,out3), dim=1))) 


class ACAM_Module(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(ACAM_Layer(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1))
    
class ACAM_Layer(nn.Module):
    def __init__(self, c1, c2, shortcut=False, g=1, e=0.5, reduction=32, gamma=2, b=0):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.gamma = gamma
        self.b = b
        t = int(abs(math.log(c1, 2) + self.b) / self.gamma)   # eca  gamma=2
        k = t if t % 2 else t - 1
        
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2, k, 1)
        self.ca = CoorAttention_ACAM(c2, c2, reduction)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        return x + self.ca(self.cv2(self.cv1(x))) if self.add else self.ca(self.cv2(self.cv1(x)))

class CoorAttention_ACAM(nn.Module):
    def __init__(self, c1, c2, reduction=32, gamma=2, b=1):
        super(CoorAttention_ACAM, self).__init__()
        ...
        self.conv1 = nn.Conv2d(c1, c_, kernel_size=k, stride=1, padding='same')
        self.bn1 = nn.BatchNorm2d(c_)
        self.act = nn.Hardswish()
        self.conv_w = nn.Conv2d(c_, c2, kernel_size=1, stride=1, padding='same')
        self.conv_h = nn.Conv2d(c_, c2, kernel_size=1, stride=1, padding='same')
